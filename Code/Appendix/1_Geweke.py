...
def run_Geweke_tests(G, graphname, sampling_gap, graphspace, allow_loops, allow_multi, is_vertex_labeled):
    m = G.number_of_edges()
    n = G.number_of_nodes()
    G_degree = list(nx.degree(G))
    Results = [graphname, m]
    ...
    iterations = 0
    print("Starting Geweke on", graphname)
    while iterations < 10: # 200
        List_edges = []
        for edge in G.edges():                    
        ...
        while found < 2:
            ...
            try:
                countchecks += 1
                x = np.array(test_r)
                scores = pymc.diagnostics.geweke(x, first=.1, last=.5, intervals=20)
                outsideCount_up = 0
                outsideCount_down = 0
                for each_tuple in scores:
                    if each_tuple[1] > 2: # or each_tuple[1] < -2:
                        outsideCount_up+=1
                for each_tuple in scores:
                    if each_tuple[1] < -2: # or each_tuple[1] < -2:
                        outsideCount_down+=1
                if outsideCount_up/len(scores) >= 0.023 or outsideCount_down/len(scores) >= 0.023:
                    found = 0 # this means we have not converged, so we keep applying more swaps.
                else:
                    found = 1 # we have converged
    ...
    if not os.path.isdir("../../Output/OtherDiagnosticTests/Geweke/" + graphspace + "/"):
        os.makedirs("../../Output/OtherDiagnosticTests/Geweke/" + graphspace + "/")
    pickleFile = "../../Output/OtherDiagnosticTests/Geweke/" + graphspace + "/" + str(graphname)+".pkl"
    pickle_out = open(pickleFile,"wb") 
    pickle.dump(Results, pickle_out)
    pickle_out.close()
    

def save_Geweke_data(G, graphname, sampling_gap, graphspace):
    ...
    run_Geweke_tests(G, graphname, sampling_gap, graphspace, allow_loops, allow_multi, is_vertex_labeled)

def store_GewekeTestResults(list_of_graphs, list_of_names, graphspace):
    ...
    for i in range(len(list_of_graphs)):
        ...
        p = multiprocessing.Process(target=save_Geweke_data, args=(G, graphname, sampling_gap, graphspace))
        jobs.append(p)
        p.start()
...
def run(graphspace):
    ...
    store_GewekeTestResults(list_of_graphs, list_of_names, graphspace)

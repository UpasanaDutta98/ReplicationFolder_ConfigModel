...
def run_GelmanRubin_tests(G, graphname, sampling_gap, graphspace, allow_loops, allow_multi, is_vertex_labeled):
    m = G.number_of_edges()
    n = G.number_of_nodes()
    G_degree = list(nx.degree(G))
    Results = [graphname, m]
    ...
    while iterations < 200:
        ...
        while found < 2:
            if found == 0:
                test_r1 = get_assortativities_and_graph(step_function, A, edge_list, swaps, allow_loops, allow_multi, is_vertex_labeled, window//2, S1, S2, S3, denominator, G_degree, n, m, last_r)
                test_r2 = get_assortativities_and_graph(step_function, A, edge_list, swaps, allow_loops, allow_multi, is_vertex_labeled, window//2, S1, S2, S3, denominator, G_degree, n, m, last_r)
            elif found == 1:
                incrementwindow = m
                test_r1 = get_assortativities_and_graph(step_function, A, edge_list, swaps, allow_loops, allow_multi, is_vertex_labeled, incrementwindow//2, S1, S2, S3, denominator, G_degree, n, m, last_r)
                test_r2 = get_assortativities_and_graph(step_function, A, edge_list, swaps, allow_loops, allow_multi, is_vertex_labeled, incrementwindow//2, S1, S2, S3, denominator, G_degree, n, m, last_r)
                break
            countchecks += 1
            try:
                x1 = np.array(test_r1)
                x2 = np.array(test_r2)
                X = np.array((x1,x2))
                R_HAT = pymc.diagnostics.gelman_rubin(X)
                if R_HAT > 1.1:
                    found = 0
                else:
                    found = 1
        ...
        swaps_done.append((window*countchecks) + m)
        r = test_r2[-1]
    ...
    if not os.path.isdir("../../Output/OtherDiagnosticTests/GelmanRubin/" + graphspace + "/"):
        os.makedirs("../../Output/OtherDiagnosticTests/GelmanRubin/" + graphspace + "/")
    pickleFile = "../../Output/OtherDiagnosticTests/GelmanRubin/" + graphspace + "/" + str(graphname)+".pkl"
    pickle_out = open(pickleFile,"wb") 
    pickle.dump(Results, pickle_out)
    pickle_out.close()


def save_GelmanRubin_data(G, graphname, sampling_gap, graphspace):
    ...
    run_GelmanRubin_tests(G, graphname, sampling_gap, graphspace, allow_loops, allow_multi, is_vertex_labeled)

def store_GelmanRubinTestResults(list_of_graphs, list_of_names, graphspace):
    ...
    for i in range(len(list_of_graphs)):
        ...
        p = multiprocessing.Process(target=save_GelmanRubin_data, args=(G, graphname, sampling_gap, graphspace))
        jobs.append(p)
        p.start()
...
def run(graphspace):
    ...
    store_GelmanRubinTestResults(list_of_graphs, list_of_names, graphspace)
